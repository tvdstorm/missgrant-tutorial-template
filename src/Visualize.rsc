...
public void renderController(Controller ctl) {
...
public Figure stateMachineGraph(TransRel trans, str init, str state) {
	  ...
	  list[Figure] nodes = [ box(text(i), fillColor(getColor(i)), grow(1.2), id(i))  
	                         | i <-[init] + toList(domain(trans) - init) ] 
	       + [ ellipse(text(labelS), grow(1.1), id("<fromS>,<labelS>,<toS>"))  
